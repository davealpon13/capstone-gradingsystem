...
<div class="col-lg-12">
    ...
    <div class="row">
        <div class="col-md-3">
            ...
            <input type="text" class="form-control form-control-sm text-right number" id="assignment" maxlength="6">
        </div>
        <div class="col-md-3">
            <label for="quiz">Quiz</label>
            <input type="text" class="form-control form-control-sm text-right number" id="quiz" maxlength="6">
        </div>
        <div class="col-md-3">
            <label for="activities">Activities</label>
            <input type="text" class="form-control form-control-sm text-right number" id="activities" maxlength="6">
        </div>
        <div class="col-md-3">
            <label for="attendance">Attendance</label>
            <input type="text" class="form-control form-control-sm text-right number" id="attendance" maxlength="6">
        </div>
    </div>
    ...
    <div class="row mt-3">
        <div class="col-md-3">
            ...
            <input type="text" class="form-control form-control-sm text-right number" id="exam" maxlength="6">
        </div>
        <div class="col-md-3">
            <button class="btn btn-primary btn-sm mt-4" type="button" id="calculate_average">Calculate Average</button>
        </div>
    </div>
</div>
...
<script>
   $('#calculate_average').click(function(){
    var assignment = parseFloat($('#assignment').val()) || 0;
    var quiz = parseFloat($('#quiz').val()) || 0;
    var activities = parseFloat($('#activities').val()) || 0;
    var attendance = parseFloat($('#attendance').val()) || 0;
    var exam = parseFloat($('#exam').val()) || 0;

    var total = assignment + quiz + activities + attendance + exam;
    var numberOfItems = (assignment !== 0) + (quiz !== 0) + (activities !== 0) + (attendance !== 0) + (exam !== 0);
    var average = total / numberOfItems;

    $('#average').text(average.toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2 }));
    $('[name="marks_percentage"]').val(average.toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2 }));
    
    // Populate the average into the input field with ID "mark"
    $('#mark').val(average.toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2 }));
});
