...
if (isset($_POST['action']) && $_POST['action'] == 'delete') {
    if (isset($_POST['id'])) {
        ...
        $query = "DELETE FROM lab_finals WHERE id='$id'";
...
if (isset($_POST['action']) && $_POST['action'] == 'deleteAll') {
    $query = "DELETE FROM lab_finals WHERE section='$section' AND subject='$subject'";
    $result = mysqli_query($conn, $query);
...
<head>
    ...
    <style>
        ...
        th, td {
            border: 1px solid #000000;/* Use a slightly different shade for borders */
            padding: 15px;
            text-align: left;
        }
...
<body>
    ...
    <td style="text-align: center; background-color: #A7C7E7; color: black; width: 45.8%; font-weight: bold;">Attendance and Participation Total</td>
    ...
    <td style="text-align: center; background-color: #fdfd96; color: black; width: 45.8%; font-weight: bold;">Edit</td>
        ...
        $i = 1;
        $rows = mysqli_query($conn, "SELECT * FROM lab_finals WHERE section='$section' AND subject='$subject'");
    ...
    function editRecord(id) {
        window.location.href = 'editLabFinals.php?id=' + id;
    }
</script>
  ...
  if ($sheetIndex !== -1) {
    ...
    foreach ($reader as $key => $row) {
        ...
        $attendanceParticipation = isset($row[28]) ? $row[28] : null;
        $weightedAttendanceParticipation = isset($row[29]) ? $row[29] : null;
        $lab1Value = isset($row[31]) ? $row[31] : null;
        $lab2Value = isset($row[32]) ? $row[32] : null;
        $lab3Value = isset($row[33]) ? $row[33] : null;
        $lab4Value = isset($row[34]) ? $row[34] : null;
        $lab5Value = isset($row[35]) ? $row[35] : null;
        $lab6Value = isset($row[36]) ? $row[36] : null;
        $lab7Value = isset($row[37]) ? $row[37] : null;
        $lab8Value = isset($row[38]) ? $row[38] : null;
        $lab9Value = isset($row[39]) ? $row[39] : null;
        $lab10Value = isset($row[40]) ? $row[40] : null;
        $totalLab = isset($row[41]) ? $row[41] : null;
        $weightedLab = isset($row[42]) ? $row[42] : null;
        $practical1Value = isset($row[44]) ? $row[44] : null;
        $practical2Value = isset($row[45]) ? $row[45] : null;
        $practical3Value = isset($row[46]) ? $row[46] : null;
        $practical4Value = isset($row[47]) ? $row[47] : null;
        $practical5Value = isset($row[48]) ? $row[48] : null;
        $totalPractical = isset($row[49]) ? $row[49] : null;
        $weightedPractical = isset($row[50]) ? $row[50] : null;
        ...
        if (!empty($studentCode) && !empty($lastname)&& $studentCode !== 'Student Number') {
            ...
            mysqli_query($conn, "INSERT INTO lab_finals 
